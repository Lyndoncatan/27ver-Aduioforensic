...
export default function SonarView({
  ...
  const draw3DVoxel = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // Grid Gradient (Floor Glow)
    const cx = width / 2;
    const cy = height / 2;
    const grd = ctx.createRadialGradient(cx, cy + 100, 0, cx, cy + 100, width);
    ...
    const project = (x: number, y: number, z: number) => {
      ...
      const scale = fl / (fl + z + 200); // Camera offset Z=200
      const x2D = width / 2 + x * scale;
      const y2D = height / 2 + y * scale; // Center Y
    ...
    // Scan Plane Calculation
    const time = Date.now() / 1000;
    const ringOffset = (time * 20) % 50; // Speed 20px/s

    // Draw Floor Grid (Retro-futuristic)
    ctx.lineWidth = 1;
    ctx.strokeStyle = "rgba(99, 102, 241, 0.1)"; // Indigo faint
    ctx.beginPath();

    // Draw expanding circles on floor for depth perception
    for (let r = ringOffset; r <= 600; r += 50) {
      if (r < 10) continue;
      // Approximate circle in 3D (ellipse)
    ...
    // Draw Scanning Plane (Moving Grid Line)
    /*
    ctx.beginPath();
    const scanYPos = 100 - scanY; // Invert for Y-up visual
    ctx.strokeStyle = "rgba(16, 185, 129, 0.3)"; // Emerald scan
    // Draw a quad for the scan plane? Maybe just lines
    */
